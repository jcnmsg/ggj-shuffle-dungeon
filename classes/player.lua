...
function Player:draw()
    ...
    if debug == true then
        ...
        screen.print(10, 0, "Health:"..self.attrs.life.." | Mana:"..self.attrs.mana.." | SDMG:"..self.attrs.sdmg.." | ARM:"..self.attrs.arm.." | EVA:"..self.attrs.eva, 0.5, CONST.white, CONST.transparent);
        for i=1, table.getn(self.cards) do
            screen.print(10, 10*i, self.cards[i].type..','..self.cards[i].mod, 0.5, CONST.white, CONST.transparent)
        end
    end
end
...
function Player:add_card(card)
    table.insert(self.cards, lua_shallowcopy(card));
end